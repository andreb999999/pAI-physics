"""
ResultsAnalysisAgent implementation using smolagents framework.
"""
...
class ResultsAnalysisAgent(BaseResearchAgent):
    """
    Agent specialized in results review and follow-up decisions.
    """

    def __init__(self, model, workspace_dir: Optional[str] = None, **kwargs):
        if workspace_dir:
            workspace_dir = os.path.abspath(workspace_dir)
            self.agent_folder = os.path.join(workspace_dir, "results_analysis_agent")
        ...
        if OpenDeepSearchTool is not None:
            ...
            print(
                "⚠️ OpenDeepSearchTool disabled for ResultsAnalysisAgent: "
        ...
        system_prompt = get_results_analysis_system_prompt(
            tools=tools,
            managed_agents=None,
        )
