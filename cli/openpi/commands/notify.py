"""openpi notify — configure and test notifications."""
...
from openpi.core.config_manager import get_config_dir, set_value
...
def notify() -> None:
    ...
    \b
    Examples:
      openpi notify setup    # Interactive notification setup
      openpi notify test     # Send a test message
    """
...
def notify_test(ctx: click.Context, channel: str) -> None:
    ...
    if channel == "telegram":
        ...
        if not token or not chat_id:
            console.print("[red]Telegram not configured.[/] Run [bold]openpi notify setup[/]")
            return
        ...
        data = json.dumps({"chat_id": chat_id, "text": "OpenPI test notification"}).encode()
        req = urllib.request.Request(url, data=data, headers={"Content-Type": "application/json"})
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                if resp.status == 200:
                    console.print("[green]Test message sent to Telegram![/]")
                else:
        ...
        if not webhook:
            console.print("[red]Slack not configured.[/] Run [bold]openpi notify setup[/]")
            return

        import urllib.request
        import json

        data = json.dumps({"text": "OpenPI test notification"}).encode()
        req = urllib.request.Request(webhook, data=data, headers={"Content-Type": "application/json"})
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                if resp.status == 200:
                    console.print("[green]Test message sent to Slack![/]")
                else:
