"""openpi campaign — manage autonomous research campaigns."""
...
def campaign() -> None:
    ...
    \b
    Examples:
      openpi campaign init                  # Create a new campaign
      openpi campaign start my_campaign     # Launch it
      openpi campaign status my_campaign    # Check progress
    """
...
def campaign_init(name: str, task: str, budget: int, output_dir: str | None) -> None:
    ...
    console.print(f"[green]Campaign created:[/] {campaign_file}")
    console.print(f"  Launch with: [bold]openpi campaign start {campaign_file}[/]")
...
def campaign_start(campaign_file: str) -> None:
    ...
    try:
        proc = subprocess.run(
            ...
            cwd=_find_openpi_root(),
        )
        if proc.returncode == 0:
            console.print("[green]Campaign initialized and first heartbeat complete.[/]")
        else:
            console.print(f"[red]Campaign start failed[/] (exit code {proc.returncode})")
    except FileNotFoundError:
        console.print("[red]Error:[/] Could not find campaign_heartbeat.py. Are you in the OpenPI directory?")
...
def campaign_status(campaign_file: str) -> None:
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            console.print(f"[red]Error:[/] {proc.stderr}")
    except FileNotFoundError:
        console.print("[red]Error:[/] campaign_cli.py not found.")
...
def campaign_repair(campaign_file: str, stage_id: str) -> None:
    ...
    try:
        ...
        if proc.returncode == 0:
            console.print(f"[green]Repair triggered for stage {stage_id}[/]")
        else:
...
def _find_openpi_root() -> str | None:
    """Try to find the OpenPI project root."""
    candidates = [
        Path.cwd(),
        Path.cwd().parent,
    ]
