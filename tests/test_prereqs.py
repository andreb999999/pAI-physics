"""
Tests for consortium/prereqs.py — LaTeX binary resolution.
"""
...
from consortium.prereqs import resolve_executable, check_latex_prereqs
...
class TestResolveExecutable:
    def test_finds_via_env_var(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CONSORTIUM_PDFLATEX_PATH", str(fake_bin))
        path, err = resolve_executable("pdflatex", "CONSORTIUM_PDFLATEX_PATH")
        assert path == str(fake_bin)
        assert err is None

    def test_env_var_points_to_missing_file(self, monkeypatch):
        monkeypatch.setenv("CONSORTIUM_PDFLATEX_PATH", "/nonexistent/pdflatex")
        path, err = resolve_executable("pdflatex", "CONSORTIUM_PDFLATEX_PATH")
        assert path is None
    ...
    def test_not_found_returns_error(self, monkeypatch):
        monkeypatch.delenv("CONSORTIUM_PDFLATEX_PATH", raising=False)
        path, err = resolve_executable(
            "definitely_not_a_real_binary_xyz123",
            "CONSORTIUM_PDFLATEX_PATH",
        )
        assert path is None
