"""openpi config — view and manage configuration."""
...
from openpi.core.config_manager import (
    get_config_dir,
    get_value,
    load_config,
    set_value,
...
@click.group()
def config() -> None:
    """View and manage OpenPI configuration.

    \b
    Examples:
      openpi config list                  # Show all settings
      openpi config get model             # Get a specific value
      openpi config set model gpt-5       # Change a setting
      openpi config edit                  # Open in $EDITOR
    """
...
def config_set(ctx: click.Context, key: str, value: str) -> None:
    ...
    \b
    Examples:
      openpi config set model claude-opus-4-6
      openpi config set budget_usd 50
      openpi config set notifications.telegram.enabled true
    """
    config_dir = ctx.obj.get("config_dir")
...
def config_edit(ctx: click.Context) -> None:
    ...
    if not cfg_path.exists():
        console.print("[yellow]No config file yet.[/] Run [bold]openpi setup[/] first.")
        raise SystemExit(1)
