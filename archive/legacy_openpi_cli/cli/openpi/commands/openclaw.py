"""openpi openclaw — manage OpenClaw autonomous oversight."""
...
def openclaw() -> None:
    ...
    \b
    Examples:
      openpi openclaw setup    # Configure OpenClaw
      openpi openclaw start    # Start the gateway
      openpi openclaw status   # Check gateway health
    """
...
def openclaw_setup(ctx: click.Context) -> None:
    ...
    console.print(f"\n[green]OpenClaw config saved to {config_file}[/]")
    console.print("  Start with: [bold]openpi openclaw start[/]")
...
def openclaw_start(background: bool) -> None:
    ...
    if launch_script:
        ...
        if background:
            ...
            console.print("[green]Gateway starting in background.[/]")
        else:
...
def openclaw_status() -> None:
    ...
    if not config_file.exists():
        console.print("[yellow]OpenClaw not configured.[/] Run [bold]openpi openclaw setup[/]")
        return
    ...
    try:
        ...
        sock.close()
        if result == 0:
            console.print(f"  Status: [green]running[/] (port {port} open)")
        else:
