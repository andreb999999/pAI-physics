"""openpi runs — list past research runs."""
...
def runs(limit: int, results_dir: str | None) -> None:
    ...
    \b
    Examples:
      openpi runs           # Show last 10 runs
      openpi runs -n 20     # Show last 20 runs
    """
    ...
    for d in run_dirs:
        ...
        if summary_path.exists():
            try:
                with open(summary_path, "r") as f:
                    summary = json.load(f)
                status = "[green]complete[/]" if summary.get("completed") else "[yellow]partial[/]"
        ...
        if (d / "final_paper.pdf").exists():
            status = "[green]complete (PDF)[/]"
        elif (d / "final_paper.md").exists():
            status = "[green]complete (MD)[/]"
        elif (d / "final_paper.tex").exists():
            status = "[green]complete (TeX)[/]"
