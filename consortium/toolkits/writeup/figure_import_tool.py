...
class FigureImportToolInput(BaseModel):
    plot_files: str = Field(description="JSON string containing list of plot file paths to import (from DataDiscoveryTool output)")
    selection_criteria: Optional[str] = Field(default=None, description="Selection criteria: 'training_curves', 'comparisons', 'ablations', 'all', or 'auto' (default: 'auto')")
    max_figures: Optional[int] = Field(default=None, description="Maximum number of figures to import (default: 6)")


class FigureImportTool(BaseTool):
    name: str = "figure_import_tool"
    description: str = """
    ...
    """
    args_schema: Type[BaseModel] = FigureImportToolInput
    ...
    def _run(self, plot_files: str, selection_criteria: Optional[str] = None, max_figures: Optional[int] = None) -> str:
        """
