...
class RunExperimentToolInput(BaseModel):
    idea_json: str = Field(description="A JSON string representing the research idea, matching the output format of GenerateIdeaTool.")
    code_model: Optional[str] = Field(default=None, description="LLM model to use for code generation (default: from RUN_EXPERIMENT_CODE_MODEL env or 'gpt-5')")
    feedback_model: Optional[str] = Field(default=None, description="LLM model to use for feedback and evaluation (default: from RUN_EXPERIMENT_FEEDBACK_MODEL env or 'gpt-5')")
    vlm_model: Optional[str] = Field(default=None, description="VLM model to use for visual feedback on plots (default: from RUN_EXPERIMENT_VLM_MODEL env or 'gpt-5')")
    report_model: Optional[str] = Field(default=None, description="LLM model to use for report generation (default: from RUN_EXPERIMENT_REPORT_MODEL env or 'gpt-5')")
    end_stage: Optional[int] = Field(default=None, description="Final stage to run (1-4). Stage 1: initial_implementation (basic working baseline), Stage 2: baseline_tuning (hyperparameter optimization), Stage 3: creative_research (novel improvements), Stage 4: ablation_studies (systematic component analysis). Default: 4 (run all stages)")
...
class RunExperimentTool(BaseTool):
    ...
    args_schema: Type[BaseModel] = RunExperimentToolInput
    ai_scientist_path: Optional[str] = None
    workspace_dir: Optional[str] = None

    def __init__(self, workspace_dir=None, **kwargs: Any):
        """
        ...
        """
        # The tool is in consortium/toolkits, so we go up two levels to the repo root
        # and then into external_tools/run_experiment_tool.
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        ai_scientist_path = os.path.join(repo_root, "external_tools", "run_experiment_tool")
        # Convert workspace_dir to absolute path immediately to avoid issues when working directory changes
        workspace_dir_abs = os.path.abspath(workspace_dir) if workspace_dir else None
        super().__init__(
            ai_scientist_path=ai_scientist_path,
            ...
            if self.workspace_dir and log_dir:
                ...
                if run_dir.startswith(self.workspace_dir):
                    ...
                    try:
                        ...
                        print(f"Warning: Could not copy full experiment results: {e}")
